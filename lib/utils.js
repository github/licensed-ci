...
async function ensureBranch(branch, parent) {
  ...
  let exitCode = await exec.exec('git', ['checkout', `${ORIGIN}/${branch}`], { ignoreReturnCode: true });
  if (exitCode != 0 && branch !== parent) {
    await exec.exec('git', ['checkout', `${ORIGIN}/${parent}`]);
  ...
  if (branch !== parent) {
    exitCode = await exec.exec('git', ['rebase', `${ORIGIN}/${parent}`, `${ORIGIN}/${branch}`], { ignoreReturnCode: true });
    if (exitCode !== 0) {
