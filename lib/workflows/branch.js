...
async function notifyStatus(octokit, licensesPullRequest, statusResult) {
  const actor = process.env.GITHUB_ACTOR;
...
// licenses branch/PR
async function notifyUser(octokit, branch, licensesBranch, licensesPullRequest, created) {
  const body = NOTIFY_USER_TEMPLATE.replace('<pr url>', licensesPullRequest.html_url)
                                   .replace('<action>', ACTIONS[created]);

  const pullRequest = await utils.findPullRequest(octokit, { head: branch, "-base": licensesBranch });
  if (pullRequest) {
    await octokit.issues.createComment({
     ...github.context.repo,
...
async function run() {
  ...
  if (branch !== licensesBranch) {
    ...
    if (exitCode > 0) {
      ...
      await notifyStatus(octokit, licensesPullRequest, statusResult);
      await notifyUser(octokit, branch, licensesBranch, licensesPullRequest, pullRequestCreated);
    }
