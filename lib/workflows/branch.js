...
async function status() {
  ...
  const exitCode = await exec.exec(command, ['status', '-c', configFilePath], { ignoreReturnCode: true });
  if (exitCode == 0) {
    return;
  }

  if (branch !== licensesBranch) {
    throw new Error(`License checks failed on ${branch}, please see changes to ${licensesBranch}`);
  } else {
    throw new Error('License checks failed');
  }
}
